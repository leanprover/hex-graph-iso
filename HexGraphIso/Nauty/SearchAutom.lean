...
@[expose] def autPruned (nn : Nat) (gens : List (Array Nat))
  ...
  (List.range o).any fun o' =>
    gens.any fun γ =>
      checkCellsPerm
        (breakout nn rsLab rsPtn (level + 1) tc rsLab[tc + o]!).2.1
        (breakout nn rsLab rsPtn (level + 1) tc rsLab[tc + o']!).1
        ((breakout nn rsLab rsPtn (level + 1) tc rsLab[tc + o]!).1.map
          fun w => γ[w]!)
        (level + 1) nn
...
@[expose] def stepA (ctx : Ctx n) (tcLevel : Nat)
    (gens : List (Array Nat)) (fuel level : Nat) (rs : RefineSt n)
    (tail0 : Option (Key n)) : Key n :=
  if discreteAt rs.ptn level n then
    ...
    let t := ((List.range tcr.2.2).filter fun o =>
        !autPruned n gens rs.lab rs.ptn level tcr.1 o).foldl
...
theorem incMax_cons (lc : Nat) (tail0 : Option (Key n)) (cs : List Nat) (rws : List (VSet n)) :
    incMax (tail0.map fun t => (⟨lc :: t.codes, t.rows⟩ : Key n))
        ⟨lc :: cs, rws⟩ =
...
theorem surviving_key {p : Nat → Bool} {key : Nat → Key n} {m : Nat}
...
theorem keysMax_head_filter {key : Nat → Key n} {p : Nat → Bool} {m : Nat}
...
theorem childKey_of_autPruned {ctx : Ctx n}
    ...
    (hpr : autPruned n gens rsLab rsPtn level tc o = true) :
    ∃ o', o' < o ∧
...
both sides. -/
theorem stepA_eq {ctx : Ctx n} (hgsz : ctx.g.size = n)
    {gens : List (Array Nat)}
    (hv : ∀ γ ∈ gens, checkAutom ctx.g γ = true) (tcLevel : Nat) :
  ...
  | fuel, level, lab, ptn, active, numcells, tail0 => by
    ...
    · -- live node: sweep the target cell, skipping pruned positions
      have hstR := refine_stOk (ctx := ctx) (active := active) (level := level)
        (numcells := numcells) hok.labSize hok.labOk hok.ptnSize hok.ptnEnd
      have hRvals : ∀ q : Nat,
      ...
      have hpt : ∀ (acc : Option (Key n)), ∀ x ∈ (0 : Nat) ::
        ...
        exact congrArg some (searchNodeA_eq hgsz hv tcLevel fuel
          (level + 1) _ _ _ _ acc hokc (by omega))
      rw [foldl_incMax_cons hpt tail0]
      have hcov : ∀ o, o < m + 1 →
          (fun o => !autPruned n gens
            (refine ctx level lab ptn active numcells).lab
            (refine ctx level lab ptn active numcells).ptn level
            p.1 o) o = false →
          ∃ o', o' < o ∧
        ...
        intro o hom hpf
        have hpf' : (!autPruned n gens
            (refine ctx level lab ptn active numcells).lab
            (refine ctx level lab ptn active numcells).ptn level
            p.1 o) = false := hpf
        have hpr : autPruned n gens
            (refine ctx level lab ptn active numcells).lab
            (refine ctx level lab ptn active numcells).ptn level
            p.1 o = true := by
          rcases hb : autPruned n gens
              (refine ctx level lab ptn active numcells).lab
              (refine ctx level lab ptn active numcells).ptn level
              p.1 o with _ | _
          · rw [hb] at hpf'
            cases hpf'
          · rfl
        exact childKey_of_autPruned hgsz hv tcLevel fuel level
...
theorem searchNodeA_eq {ctx : Ctx n}
    (hgsz : ctx.g.size = n) {gens : List (Array Nat)}
    (hv : ∀ γ ∈ gens, checkAutom ctx.g γ = true) (tcLevel : Nat) :
    ∀ (fuel level : Nat) (lab ptn : Array Nat)
  ...
  | fuel + 1, level, lab, ptn, active, numcells, none => by
    intro hok hlf
    rw [searchNodeA,
      stepA_eq hgsz hv tcLevel fuel level lab ptn active numcells
        none hok hlf]
    rfl
  | fuel + 1, level, lab, ptn, active, numcells,
    ...
    rw [searchNodeA]
    simp only []
    rw [stepA_eq hgsz hv tcLevel fuel level lab ptn active numcells
      none hok hlf]
  ...
  | fuel + 1, level, lab, ptn, active, numcells,
    ...
    · -- equal codes: descend, threading the incumbent's tail
      ...
      simp only []
      simp only [hcmp]
      rw [stepA_eq hgsz hv tcLevel fuel level lab ptn active
...
@[expose] def searchCanonA (n : Nat) (gens : List (Array Nat))
    (g : Array (VSet n)) (lab0 : Array Nat) (cellEnds : List Nat) : Key n :=
  if n == 0 then
    ⟨[], []⟩
  else
    searchNodeA { g } 100 gens n 1 lab0 (initPtn n (n + 2) cellEnds)
...
theorem searchCanonA_key (G : Colored n k) {gens : List (Array Nat)}
    (hv : ∀ γ ∈ gens, checkAutom (rowsOf G) γ = true) :
  ...
  · rw [ite_eq_right (by simp; omega), ite_eq_right (by simp; omega)]
    exact searchNodeA_eq (ctx := { g := rowsOf G })
