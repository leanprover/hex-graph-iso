...
- `testcanlab_fst` / `testcanlab_prefix` / `testcanlab_snd_le`:
  `testcanlab` returns the trichotomy of the lexicographic `rowCmp`
...
- `leafEvent_faithful`: the packaged per-leaf clause — the comparison
  outcome is `listCmp rowCmp (leafRows ctx lab) (leafRows ctx
...
key's row list. -/
@[expose] def leafRow (ctx : Ctx) (lab : Array Nat) (i : Nat) : Nat :=
  permset ctx.g[lab[i]!]! (invPerm lab) ctx.n

theorem leafRows_eq_map (ctx : Ctx) (lab : Array Nat) :
    leafRows ctx lab = (List.range ctx.n).map (leafRow ctx lab) := rfl
...
`canonlab`. -/
def CanongInv (ctx : Ctx) (canong canonlab : Array Nat)
    (samerows : Nat) : Prop :=
  canong.size = ctx.n ∧
...
theorem rows_of_canongInv {ctx : Ctx} {canong canonlab : Array Nat}
    (hinv : CanongInv ctx canong canonlab ctx.n) :
    (List.range ctx.n).map (canong[·]!) = leafRows ctx canonlab := by
  rw [leafRows_eq_map]
...
private theorem forIn_set_eq_foldl (ctx : Ctx) (lab : Array Nat) :
    ∀ (L : List Nat) (g : Array Nat),
      (forIn L g (fun i s =>
...
private theorem updatecan_eq_foldl (ctx : Ctx) (canong lab : Array Nat)
    (samerows : Nat) :
    updatecan ctx canong lab samerows =
      (List.range' samerows (ctx.n - samerows)).foldl
        (fun a i => a.set! i (leafRow ctx lab i)) canong := by
  have h0 : updatecan ctx canong lab samerows =
      (forIn [samerows:ctx.n] canong (fun i s =>
          pure (ForInStep.yield (s.set! i (leafRow ctx lab i)))) :
        Id (Array Nat)) := rfl
  rw [h0, forIn_range'_eq]
  exact forIn_set_eq_foldl ctx lab _ canong

private theorem foldl_set_size (ctx : Ctx) (lab : Array Nat) :
    ∀ (len s : Nat) (g : Array Nat),
...
private theorem foldl_set_getElem! (ctx : Ctx) (lab : Array Nat) :
    ∀ (len s : Nat) (g : Array Nat) (j : Nat),
      ((List.range' s len).foldl
...
theorem updatecan_inv {ctx : Ctx} {canong canonlab : Array Nat}
    {samerows : Nat} (hinv : CanongInv ctx canong canonlab samerows) :
...
private def tclHit (ctx : Ctx) (canong lab : Array Nat) (i : Nat) :
    Option (Int × Nat) :=
  match rowCmp (leafRow ctx lab i) canong[i]! with
  | .lt => some (-1, i)
  | .gt => some (1, i)
  | .eq => none
...
private theorem testcanlab_eq_scan (ctx : Ctx) (canong lab : Array Nat) :
    testcanlab ctx canong lab =
      Break.runK.match_1 (fun _ => Int × Nat)
        ...
        (fun r => r) (fun _ => (0, ctx.n)) := by
  have h0 : testcanlab ctx canong lab =
      Break.runK.match_1 (fun _ => Int × Nat)
        (forIn [0:ctx.n] (none, ()) (tclBody ctx canong lab) :
          Id (Option (Int × Nat) × Unit)).fst
        (fun r => r) (fun _ => (0, ctx.n)) := rfl
  rw [h0, forIn_range_eq]

private theorem tclBody_gate (ctx : Ctx) (canong lab : Array Nat)
    (i : Nat) :
    HitGate (tclBody ctx canong lab i) (tclHit ctx canong lab i) := by
  cases hcmp : rowCmp (leafRow ctx lab i) canong[i]! with
  | lt =>
...
private theorem testcanlab_eq_firstHit (ctx : Ctx)
    (canong lab : Array Nat) :
    testcanlab ctx canong lab =
      (firstHit (tclHit ctx canong lab)
        (List.range ctx.n)).getD (0, ctx.n) := by
  rw [testcanlab_eq_scan,
    forIn_scan_hits (List.range ctx.n) (tclBody ctx canong lab)
      (tclHit ctx canong lab) (tclBody_gate ctx canong lab)]
  cases firstHit (tclHit ctx canong lab) (List.range ctx.n) <;> rfl
...
private theorem firstHit_trichotomy (ctx : Ctx)
  ...
  | succ m ih =>
    intro s
    rw [List.range'_succ]
    cases hcmp : rowCmp (leafRow ctx lab s) canong[s]! with
    | eq =>
      have hhead : leafRow ctx lab s = canong[s]! :=
        rowCmp_eq_iff.mp hcmp
...
theorem testcanlab_fst (ctx : Ctx) (canong lab : Array Nat) :
    (testcanlab ctx canong lab).1 =
      ordInt (listCmp rowCmp (leafRows ctx lab)
        ((List.range ctx.n).map (canong[·]!))) := by
  rw [testcanlab_eq_firstHit, leafRows_eq_map, List.range_eq_range']
  rcases firstHit_trichotomy ctx canong lab ctx.n 0 with
...
theorem testcanlab_prefix (ctx : Ctx) (canong lab : Array Nat) :
  ...
  · rw [hfh]
    show ∀ i, i < ctx.n → leafRow ctx lab i = canong[i]!
...
theorem testcanlab_snd_le (ctx : Ctx) (canong lab : Array Nat) :
    (testcanlab ctx canong lab).2 ≤ ctx.n := by
  rw [testcanlab_eq_firstHit, List.range_eq_range']
  rcases firstHit_trichotomy ctx canong lab ctx.n 0 with
...
the fresh leaf — re-establishing `CanongInv` whichever way the leaf
resolves. -/
theorem leafEvent_faithful {ctx : Ctx} {canong canonlab lab : Array Nat}
    {samerows : Nat}
...
theorem keyCmp_codes_eq {cs r1 r2 : List Nat} :
    keyCmp ⟨cs, r1⟩ ⟨cs, r2⟩ = listCmp rowCmp r1 r2 := by
  rw [keyCmp,
