...
/-- Later sibling sweeps preserve the root pruning workspace. Each child
return restores its path, boundary and trace histories before another sibling
can classify or admit a pair; skipped vertices require only filter inclusion. -/
theorem pairs_sweep (G : GraphIso.Sparse.Colored n k) (hn : 0 < n) (tcLevel fuel : Nat)
    (hd : ∀ level numcells st, 1 ≤ level → PairsEntry G tcLevel level numcells st →
      PairsOk G (Generic.node false (.ofGraph G.graph) (n + 2) tcLevel fuel level numcells st).2)
    (cfuel : Nat) (first : Bool) (level numcells tc tv1 index : Nat)
    (cursor : Option Nat) (cell : VSet n) (st : State n) (hl : 1 ≤ level)
    (h : PairsReady G tcLevel level numcells st) (ht : Generic.Target State.frame level tc cell st)
    (hv : ∀ v, cursor = some v → cell.mem v = true) (hpast : Generic.Past first tv1 cursor)
    (hrecord : CheapRecorded level tc st) :
    PairsOk G (Generic.sweep first (.ofGraph G.graph) (n + 2) tcLevel fuel cfuel level numcells
      tc tv1 cursor cell index st).2.2 := by
  induction cfuel generalizing cursor cell index st with
  | zero => cases cursor <;> rw [Generic.sweep] <;> exact h.pairs
  | succ cfuel ih =>
    cases cursor with
    | none => rw [Generic.sweep]; exact h.pairs
    | some tv =>
      have hmem := hv tv rfl
      ...
      have hpairs : PairsOk G raw.2 := hd _ _ _ (by omega) hch
      have hback := h.child_return hn hl first fuel ht hmem hrecord hpairs
      ...
      have hcontinue : ∀ smaller, (∀ v, smaller.mem v = true → cell.mem v = true) → ∀ index,
        ...
        exact ih index _ smaller _ hback.1 ((ht.subset hs).of_out hrestore.effect)
          (fun _ hv => VSet.nextElem_mem hv) (hp smaller) hback.2
      have hresume : ∀ smaller, (∀ v, smaller.mem v = true → cell.mem v = true) →
          PairsOk G (Generic.resume (n + 2) next first level numcells tc tv1 tv smaller index left).2.2 := by
        intro smaller hs
      ...
      have hadv : PairsOk G (Generic.advance (n + 2) next first level numcells tc tv1 tv cell index left raw.1).2.2 := by
        cases raw.1 with
        | fuel => exact hpairs.leave tv
        | done => exact hresume cell (fun _ hv => hv)
        | unwind target short =>
          unfold Generic.advance
          simp only [Id.run_pure, apply_ite Id.run, apply_ite Prod.snd]
          split
          · exact hpairs.leave tv
          · split
