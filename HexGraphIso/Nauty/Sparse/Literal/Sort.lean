...
namespace Hex.GraphIso.Nauty.Sparse.Literal.Sort
...
def partition (x y : Array Nat) (start len : Nat) : Array Nat × Nat × Nat := Id.run do
  ...
  for i in range (0) (left) do x := x.swapIfInBounds (start + i) (b - left + i)
  let right := min dc (start + len - d)
  for i in range (0) (right) do x := x.swapIfInBounds (b + i) (start + len - right + i)
  return (x, ba, dc)
...
theorem median_eq : median = Sparse.Sort.median := rfl

theorem insertion_eq : insertion = Sparse.Sort.insertion := by
  funext x y start len
  simp only [insertion, Sparse.Sort.insertion, range_forIn]

theorem pivot_eq : pivot = Sparse.Sort.pivot := rfl

theorem partition_eq : partition = Sparse.Sort.partition := by
  funext x y start len
  simp only [partition, Sparse.Sort.partition, pivot_eq, range_forIn]
  rfl

theorem indirect_eq : indirect = Sparse.Sort.indirect := by
  funext x y start len
  simp only [indirect, Sparse.Sort.indirect, insertion_eq, partition_eq, range_forIn]
  rfl

end Hex.GraphIso.Nauty.Sparse.Literal.Sort
