...
public import HexGraphIso.Nauty.Sparse.ReachNode
...
/-- Recovery and both target filters preserve the parent frame before the
next surviving sibling. Filters need only be subsets for this reachability rule. -/
theorem reach_advance (G : GraphIso.Sparse.Colored n k) (hn : 0 < n)
  ...
  have hcontinue : ∀ smaller, (∀ v, smaller.mem v = true → cell.mem v = true) →
    ...
    exact hr.2.trans (hnext first level numcells tc tv1 (smaller.nextElem (some tv)) smaller _ _
      ⟨hl, hr.1, ht, fun _ hv => VSet.nextElem_mem hv⟩)
...
/-- The executed sparse sweep individualizes only a member of its current
target cell and composes every recursive return with its caller's frame. -/
theorem reach_sweep (G : GraphIso.Sparse.Colored n k) (hn : 0 < n)
  ...
  · generalize he : descend (first && tv == tv1) (level + 1) (numcells + 1)
      ((policy (n := n)).child first level tc tv st) = r at hd ⊢
    obtain ⟨exit, out⟩ := r
    have hparent : FrameOut G level level st out := h.child_frame hn hl first htarget htv hd
    split
    · exact reach_advance G hn hnext first level numcells tc tv1 tv index cell st _ exit hl h htarget
        ((hparent.afterChild level tv1).leave tv)
    · exact reach_advance G hn hnext first level numcells tc tv1 tv index cell st _ exit hl h htarget
        (hparent.leave tv)
  · exact hnext first level numcells tc tv1 (cell.nextElem (some tv)) cell _ st
      ⟨hl, h, htarget, fun _ hv => VSet.nextElem_mem hv⟩
