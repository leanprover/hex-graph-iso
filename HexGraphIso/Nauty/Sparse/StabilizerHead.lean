...
public import HexGraphIso.Nauty.Sparse.StabilizerTail
...
/-- The actual first sweep returns the exact size of the guide's full
point-stabilizer orbit. The first child initializes the count, and every
later mark is justified at the state where the executable reads it. -/
theorem FirstInput.index {G : GraphIso.Sparse.Colored n k}
    ...
    (Generic.sweep true (.ofGraph G.graph) (n + 2) tcLevel fuel (n + 1)
      f.level c.numcells p.tc tv (some tv) p.cell 0 p.state).2.1 =
  ...
  have hwindow : ∀ v, Aut.Orbit G.toDense base guide v → p.cell.mem v.val = true := by
    intro v hv
  ...
  have hmin : ∀ v, Aut.Orbit G.toDense base guide v → guide.val ≤ v.val :=
    fun v hv => nextElem_le htv (hwindow v hv) trivial
  have hcover : Nauty.Generation.Cover G.toDense back.generators base guide p.cell (some tv) :=
    (Nauty.Generation.Cover.start hwindow).advance (tv := guide) htv
      (fun _ => Nauty.Generation.Carries.refl G.toDense back.generators base guide)
  ...
  have hpReplay : OrbitReplay p.state :=
    (orbitReplayPolicy g (n + 2) tcLevel).cheap true f.level r.2.2.2.2
      ((orbitReplayPolicy g (n + 2) tcLevel).target true f.level r.1 _
        ((orbitReplayPolicy g (n + 2) tcLevel).record f.level (f.code G.graph) _
          ((orbitReplayPolicy g (n + 2) tcLevel).visit f.level f.numcells f.entry hreplay)))
  have hbReplay : OrbitReplay back :=
    (orbitReplayPolicy g (n + 2) tcLevel).recover f.level left
      ((orbitReplayPolicy g (n + 2) tcLevel).leave tv _
        ((orbitReplayPolicy g (n + 2) tcLevel).afterChild f.level tv _
          (node_orbitReplay true g (n + 2) tcLevel fuel ch.level ch.numcells ch.entry
            ((orbitReplayPolicy g (n + 2) tcLevel).child true f.level p.tc tv p.state hpReplay))))
  ...
  have hmark : (back.orbits[tv]! == tv) = true ↔ Aut.Orbit G.toDense base guide guide := by
    rw [beq_iff_eq]
    exact hbReplay.mark (tv := guide) hs.codes.trace hs.orbits hcover (VSet.nextElem_mem htv) hfix hmin
  have hcount := (Nauty.Generation.Counter.start (Aut.Orbit G.toDense base guide)).advance
    (tv := guide) hwindow htv hmark
  change (Generic.sweep true g (n + 2) tcLevel fuel (n + 1)
    f.level c.numcells p.tc tv (some tv) p.cell 0 p.state).2.1 = _
  rw [hsuffix]
  apply hs.index_tail (cfuel := n) (guide := guide)
    (index := if back.orbits[tv]! == tv then 1 else 0) rfl hf (fun _ _ => by omega) (by
    intro _ v hv
    have hh := (VSet.nextElem_eq_some_iff.mp hv).2.1
    change tv + 1 ≤ v at hh
    change tv < v
    omega) (hm.congr hreference) heq hfloor hboundary hmove hfixFrame hwindow hmin rfl hcanon hcover hfirst
  · exact hbReplay
  · exact hcount
