...
/-- The actual first descent installs a reference path carrying the
uniformity guarantee at its final all-same boundary. -/
theorem first_path {G : Colored n k} {ctx : Ctx n} (tcLevel : Nat)
    (hg : ctx.g = rowsOf G) :
    ∀ fuel level numcells (codes : List Nat) (st : SearchSt n) (trail : FrameTrail),
      ...
      ∃ targets key,
        RefPath ctx tcLevel (firstPathNode ctx (n + 2) tcLevel fuel level numcells st).2.allsamelevel
          level (refine ctx level st.lab st.ptn st.active numcells) targets key ∧
        Matches ctx level (firstPathNode ctx (n + 2) tcLevel fuel level numcells st).2 targets key := by
  intro fuel
  induction fuel with
  | zero =>
  ...
  | succ fuel ih =>
    ...
    by_cases hsame : (firstPathNode ctx (n + 2) tcLevel (fuel + 1) level numcells st).2.allsamelevel ≤ level
    · obtain ⟨ts, k, hu⟩ := first_uniform tcLevel hg (fuel + 1) level numcells codes st trail
        hfirst hpathOk hcheap hdesc horb hlevel hpath htcsize hfuel hsame
      obtain ⟨targets, key, hleaf, hm, _⟩ := first_reference (n + 2) tcLevel hg (fuel + 1)
        level numcells codes st trail hfirst hlevel hpath htcsize (by omega)
      have hu' : Uniform ctx tcLevel level rs targets key := by
        obtain ⟨rfl, rfl⟩ := hu targets key hleaf
        exact hu
      exact ⟨targets, key, hleaf.uniformPath hit hu', hm⟩
    · by_cases hnum : rs.numcells = n
      ...
      · have hbc : bcount rs.ptn level n < n := by
        ...
        have hchildOrb : OrbSound (OrbConn child.genTrace.toList n) child.orbits n := by
          ...
          dsimp only [pre]
          split <;> exact horb
        obtain ⟨targets, key, hchildPath, hmatches⟩ := ih (level + 1) (rs.numcells + 1) full child childTrail
        ...
        obtain ⟨_, _, _, _, hfloor⟩ := first_reference (n + 2) tcLevel hg fuel (level + 1)
          ...
          (by simp only [full, List.length_append, List.length_singleton]; omega) hctc (by omega)
        have hocc : RefPath ctx tcLevel
            (firstPathNode ctx (n + 2) tcLevel fuel (level + 1) (rs.numcells + 1) child).2.allsamelevel
            (level + 1) (childSt ctx level rs tc tv) targets key := by
          change RefPath ctx tcLevel _ (level + 1)
            (refine ctx (level + 1) (breakout n pre.lab pre.ptn (level + 1) tc tv).1
              (breakout n pre.lab pre.ptn (level + 1) tc tv).2.1
              (breakout n pre.lab pre.ptn (level + 1) tc tv).2.2 (rs.numcells + 1)) targets key at hchildPath
          rw [hplab, hpptn, breakout_ptn] at hchildPath
          exact hchildPath
        have hchildPrefix := first_prefix ctx (n + 2) tcLevel fuel (level + 1) (rs.numcells + 1) child level (by omega)
        have hcode : rs.longcode = (firstPathNode ctx (n + 2) tcLevel fuel (level + 1) (rs.numcells + 1) child).2.firstcode[level]! := by
          rw [hchildPrefix.1]
          change rs.longcode = pre.firstcode[level]!
          rw [hpcode, Array.getElem!_set!_self _ _ _ (by rw [hfirst.codes.firstSize]; omega)]
        have htc : Int.ofNat tc = (firstPathNode ctx (n + 2) tcLevel fuel (level + 1) (rs.numcells + 1) child).2.firsttc[level]! := by
          rw [hchildPrefix.2]
        ...
        have hparent : RefPath ctx tcLevel
            (firstPathNode ctx (n + 2) tcLevel fuel (level + 1) (rs.numcells + 1) child).2.allsamelevel
            level rs (tc :: targets) ⟨rs.longcode :: key.codes, key.rows⟩ :=
          .step hlt hecell (by omega) (by omega) htarget (hat ▸ hocc) (fun hb => by omega)
        ...
        have hploop : RefPath ctx tcLevel
            (firstChildLoop ctx (n + 2) tcLevel fuel (n + 1) level rs.numcells tc tv
              (some tv) tcell 0 pre).2.2.allsamelevel
            level rs (tc :: targets) ⟨rs.longcode :: key.codes, key.rows⟩ := by
          rw [hfields.same]
          exact hparent
        refine ⟨tc :: targets, ⟨rs.longcode :: key.codes, key.rows⟩, ?_⟩
        rw [firstPath_internal_state ctx (n + 2) tcLevel fuel level numcells st hnum, hmk] at hsame ⊢
        dsimp only at hsame ⊢
        ...
        change RefPath ctx tcLevel out.allsamelevel level rs (tc :: targets)
            ⟨rs.longcode :: key.codes, key.rows⟩ ∧
          Matches ctx level out (tc :: targets) ⟨rs.longcode :: key.codes, key.rows⟩
        change ¬ out.allsamelevel ≤ level at hsame
        dsimp only [out, loop] at hsame ⊢
        generalize he : firstChildLoop ctx (n + 2) tcLevel fuel (n + 1) level rs.numcells tc tv
          (some tv) tcell 0 pre = result at hmloop hploop hsame ⊢
        obtain ⟨r, index, out⟩ := result
        cases r with
        | some r => exact ⟨hploop, hmloop⟩
        | none =>
          dsimp only at hsame ⊢
          unfold firstFinish at hsame ⊢
          split
          · next hif =>
            rw [ite_eq_left hif] at hsame
            have hs := beq_iff_eq.mp hif.2
            simp only at hsame
            omega
          · exact ⟨hploop, hmloop⟩
