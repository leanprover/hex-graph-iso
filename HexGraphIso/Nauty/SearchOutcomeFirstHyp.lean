...
theorem CanonTrail.ofPerm {ctx : Ctx} {level : Nat} {st out : SearchSt}
    {trail : FrameTrail}
    (htrail : TrailOk ctx level st trail)
    (hlab : st.lab.size = ctx.n) (hptn : st.ptn.size = ctx.n)
    (hend : st.ptn[st.ptn.size - 1]! ≤ level)
    (hperm : cellsPerm st.ptn level st.lab out.canonlab)
    (hcsz : out.canonlab.size = ctx.n) :
...
structure FirstSweepHyp (G : Colored n k) (ctx : Ctx)
  ...
  park : cheapautom rsPtn level ctx.n = false → st.noncheaplevel ≠ level
...
structure FirstSweepKeep (ctx : Ctx) (level e : Nat) (fs : List Nat)
    (st out : SearchSt) (outBest : Option Key) : Prop where
...
theorem LoopInv.childDescWeak {G : Colored n k} {ctx : Ctx}
...
merely known to avoid the loop level. -/
theorem LoopInv.subtreeAtWeak {G : Colored n k} {ctx : Ctx}
...
theorem cheapOk {G : Colored n k} {ctx : Ctx}
    {tcLevel specFuel level numcells tc len tcell e tv1 : Nat}
    {codes bs fs : List Nat} {rsLab rsPtn : Array Nat}
    {cursor : Option Nat} {base st : SearchSt} {best : Option Key}
...
theorem filter {G : Colored n k} {ctx : Ctx}
    {tcLevel specFuel level numcells tc len tcell tcell' e tv1 : Nat}
    {codes bs fs : List Nat} {rsLab rsPtn : Array Nat}
    {cursor : Option Nat} {base st : SearchSt} {best : Option Key}
    {trail : FrameTrail}
    (h : FirstSweepHyp G ctx tcLevel specFuel level codes bs fs numcells
...
theorem SearchOut.inputEq {G : Colored n k} {B lev : Nat}
    {st st' out : SearchSt} (h : SearchOut G B lev st out)
...
theorem LoopInv.childCanonPerm {G : Colored n k} {ctx : Ctx}
    {tcLevel specFuel level numcells tc len tcell tv currentOffset : Nat}
    {codes bs fs : List Nat} {rsLab rsPtn : Array Nat}
    {cursor : Option Nat} {base st : SearchSt} {best : Option Key}
    ...
    (hcurrent : currentOffset < len)
    (hat : st.lab[tc + currentOffset]! = tv)
    (hcsz : canonlab.size = ctx.n)
    (hnew : cellsPerm (breakout st.lab st.ptn (level + 1) tc tv).2.1
      (level + 1) (breakout st.lab st.ptn (level + 1) tc tv).1 canonlab) :
  ...
  have hnew' : cellsPerm child.ptn (level + 1) child.lab canonlab := by
    change cellsPerm (breakout st.lab st.ptn (level + 1) tc
  ...
  have hchildPtn : child.ptn = st.ptn.set! tc (level + 1) :=
    breakout_ptn st.lab st.ptn (level + 1) tc st.lab[tc + currentOffset]!
  have hchildOk : SearchOk G (level + 1) (numcells + 1) child := by
    apply breakout_searchOk hinv.nonempty hinv.run.searchOk hinv.positive
      ...
      (by rw [← hn]; exact hinv.range) hcurrent
    · rfl
    · exact hchildPtn
    · rfl
...
theorem next {G : Colored n k} {ctx : Ctx}
    {inf tcLevel specFuel runFuel level numcells tc len tcell tv offset
      currentOffset e tv1 : Nat}
    {codes bs fs : List Nat} {rsLab rsPtn : Array Nat}
    {cursor : Option Nat} {base st out : SearchSt}
    {best childBest : Option Key} {trail eventTrail : FrameTrail} {r : Int}
    ...
    (hshort : (clearShortIf clear
      { out with fixedpts := erase out.fixedpts tv }).needshortprune =
        false) :
    ∃ bs', FirstSweepHyp G ctx tcLevel specFuel level codes bs' fs numcells
      ...
      (recover ctx.n inf level
        (clearShortIf clear { out with fixedpts := erase out.fixedpts tv }))
  ...
  have hstFirst : ({ st with
      ...
      active := (breakout st.lab st.ptn (level + 1) tc tv).2.2
      fixedpts := insert st.fixedpts tv
      cosetindex := tv } : SearchSt).gcaFirst = level := hh.guide
  ...
  have hcover : SweepCover ctx tcLevel specFuel level codes rsLab rsPtn tc
    ...
    SweepCover.receipt hh.inv.cover hnext hchild.node.toOutcome.receipt hfuel
      hret heq hoffset hatFrozen hchild.coset hgsz hgb
  ...
  rw [hat] at hpathRec
  have hrecEq : recSt = clearShortIf clear (recover ctx.n inf level cleaned) :=
    recover_clearShortIf ctx.n inf level clear cleaned
  ...
  have hcanonlabRec : recSt.canonlab = out.canonlab := by
    rw [show recSt = recover ctx.n inf level cleared from rfl, hframes.1, hcl]
  ...
  have hstLab : st.lab.size = ctx.n := by
    rw [hn]
    exact hh.inv.run.searchOk.labSize
  have hstPtn : st.ptn.size = ctx.n := by
    rw [hn]
  ...
  have hcsz : out.canonlab.size = ctx.n := by
    rw [hn]
  ...
  have hrefs : FrameRefs ctx tcLevel specFuel level codes rsLab rsPtn tc len
    ...
    · intro _
      ...
      · rw [hfirstlabRec]
        exact hperm
    · intro heqc
      have hcanonRec := recover_gcaCanon ctx.n inf level cleared
      rcases hchild.canonGuide with hold | hnew
      ...
      · have hmem : elem tcell rsLab[tc + offset]! = true := by
        ...
        have hchildAt : (breakout st.lab st.ptn (level + 1) tc tv).1[tc]! =
            tv := by
          rw [← hat, breakout_at_target hstInj hcurrentPos]
        have hchildCell : IsCell (breakout st.lab st.ptn (level + 1) tc
            tv).2.1 (level + 1) tc 1 := by
          rw [breakout_ptn]
          exact isCell_breakout_target (lab := st.lab) (tv := tv) htcPtn
  ...
  refine ⟨?_, hlive, hpath, ?_, ?_, ?_, hgfRec, hfirstTrail, hcanonTrail,
    ?_, ?_, ?_, ?_, ?_, ?_, ?_, hh.baseActive⟩
  · exact {
      nodeCount := hh.inv.nodeCount
...
theorem clearShortIf_lab (clear : Bool) (st : SearchSt) :
    (clearShortIf clear st).lab = st.lab := by
  cases clear <;> rfl

theorem clearShortIf_ptn (clear : Bool) (st : SearchSt) :
...
theorem shortPairAtReceiver {G : Colored n k} {ctx : Ctx}
...
theorem clearShortIf_self (st : SearchSt) :
...
whatever its return. -/
theorem childTrails {G : Colored n k} {ctx : Ctx}
    ...
    FirstTrail ctx level { out with fixedpts := erase out.fixedpts tv }
        eventTrail ∧
      CanonTrail ctx level { out with fixedpts := erase out.fixedpts tv }
        eventTrail := by
  have hext : TrailExt level trail eventTrail :=
    TrailExt.ofPush hchild.node.preserved
